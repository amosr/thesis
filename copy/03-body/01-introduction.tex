...
However, push streams generally do not support computations with multiple inputs except for two cases: firstly, non-deterministically merging two streams; and secondly, in some circumstances appending streams --- as we shall see in \cref{taxonomy/push}.
...
This fusion method generalises previous work on stream fusion (\cref{related/stream-fusion}) and demonstrates the connection between fusion and the synchronised product operator (\cref{related/synchronised-product}), which is generally used in the context of proving properties of concurrent processes, rather than as an optimisation.
