...
However, push streams generally do not support computations with multiple inputs except for two cases: first, non-deterministically merging two streams; and second, in some circumstances appending streams --- as we shall see in \cref{taxonomy/push}.
...
If the input data is small enough to fit in memory as an array, the cost of reading the data multiple times becomes less prohibitive, but we would still like to read the data as few times as possible.
...
Icicle runs in production in the machine-learning pipeline at Ambiata.
For one particular client, we receive thirty gigabtyes of uncompressed new data every day, over which we compute more than four thousand queries in a single pass.
The input data is compressed and kept indefinitely.
When creating a new machine-learning model, Icicle is used to provide training data by computing queries over the five terabytes of compressed historical data.
