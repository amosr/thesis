...
We focus on two streaming models: push streams (\autoref{taxonomy/push}), and Kahn process networks (\autoref{taxonomy/kpn}, \autoref{chapter:process:processes}).
...
In \autoref{s:Fusion} we introduce a method for taking multiple processes in a Kahn process network and fusing them together into a single process.
...
Icicle is described in \autoref{part:icicle}.
...
The fusion algorithm is described in \autoref{chapter:process:processes}.
...
The proof is described in \autoref{s:Proofs}.
