...
Time-dependent merge can be implemented deterministically using a non-blocking read, but using a non-blocking read allows it to \emph{observe} the non-determinism in scheduling.
This operation is sometimes known as a non-deterministic merge in terms of Kahn process networks \CITE, while in streaming languages such as \CITE it is known as a deterministic merge.
While this cannot be implemented as a Kahn process, in some cases it is possible to embed the time inside the value, and use simple value-dependent merge.
...
Only value-dependent merge is able to be expressed as a Kahn process network, it is important to compare which systems support value-dependent and which time-dependent.

\subsection{Array computations}

In our case we are interested in streams as a method for optimising array computations, by converting arrays to streams and vice versa.
In some streaming operations the time domain of stream values is interesting, but this is not true for array computations.
This makes Kahn process networks an ideal target for array computations: the restrictions help one to reason about how processes act, by allowing us to ignore the minutiae of scheduling and communications.

Many standard stream operations can be written in this way, such as maps, filters, appends, etc.
