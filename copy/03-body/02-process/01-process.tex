...
process -- map tp\_f stock timeprices
 ...
 , heap:   { map_v }
 , label:  M0
 , instrs: { M0 = pull  stock       map_v     M1[] else M3[]
           , M1 = push  timeprices (timeprices_f map_v) M2[]
           , M2 = drop  stock                           M0[]
...
\begin{process}
process -- map tp\_f stock timeprices / foldl reg\_k reg\_z reg\_j timeprices reg\_out
 { ins:    { stock  }
 , outs:   { timeprices
           , reg_out }
 , heap:   { map_v
 ...
 , instrs: { M0_F0        = pull stock map_v M1_F0[] else M3_F0[] -- M0 (LocalPull)
           , M1_F0        = push timeprices (tp_f map_v) M2_F0_pending[chan_tp = (tp_f map_v)]
           -- M1 (SharedPush)
           , M2_F0_pending= jump M2_F1_have[reg_v = chan_tp] -- F0 (SharedPullPending)
           , M2_F1_have   = drop stock M0_F1_have[] -- M2 (LocalDrop)
           , M0_F1_have   = jump M0_F0[reg_s = reg_k reg_s reg_v]
           -- F1 (ConnectedDrop)

           -- stock closed
           , M3_F0        = close timeprices M4_F0_closed[] -- M3 (SharedClose)
           , M4_F0_closed = jump M4_F2_closed[] -- F0 (SharedPullClosed)
           , M4_F2_closed = push reg_out (reg_j reg_s) M4_F3_closed -- F2 (LocalPush)
           , M4_F3_closed = close reg_out M4_F4_closed[] -- F3 (LocalClose)
           , M4_F4_closed = exit } }
\end{process}
