...
process -- map tp\_f stock timeprices
 ...
 , instrs: { M0 = pull  stock       tp_v        M1[] else M3[]
           ...
           , M4 = exit } }
\end{process}
\caption{Instantiated process for map (timeprices)}
...
process -- foldl reg\_k reg\_z reg\_j timeprices reg\_out
 ...
 , instrs: { F0 = pull  timeprices reg_v  F1[] else F2[]
           ...
           , F4 = exit } }
\end{process}
\caption{Instantiated process for fold (regression)}
...
\newcommand\FuTiReLa[3] { ((#1,\{\}),(#2,\{timeprices=#3\}))}

Next, the @map@ process pushes into
...
node[shape=none];
  I[shape=point];
  M0 [label="pull stock tp_v (M0)"];
  M1 [label="push timeprices (tp_f tp_v) (M1)"];
  M2 [label="drop stock (M2)"];
  M3 [label="close timeprices (M3)"];
  M4 [label="exit (M4)"];

  I -> M0;

  M0 -> M1 [label="have[tp_v = ...]"];
  M1 -> M2;
  M2 -> M0;
...
\end{dot2tex}
\caption{State machine for map.}
\label{figs/procs/fsm/map}
\end{figure}
...
node[shape=none];
  I[shape=point];
  F0 [label="pull timeprices reg_v (F0)"];
  F1 [label="drop timeprices (F1)"];
  F2 [label="push reg_out (reg_j reg_s) (F2)"];
  F3 [label="close reg_out (F3)"];
  F4 [label="exit (F4)"];

  I -> F0;

  F0 -> F1 [label="have[reg_v = ...]"];
  F1 -> F0 [label="[reg_s = reg_k reg_s reg_v]"];
...
\end{dot2tex}
\caption{State machine for foldl.}
\label{figs/procs/fsm/foldl}
\end{figure}
...
digraph G {
node[shape=none];
  M0F0 [label="pull stock tp_v ((M0,{}),(F0,{timeprices=none}))"];
  M1F0 [label="... ((M1,{}),(F0,{timeprices=none}))"];
  M3F0 [label="... ((M3,{}),(F0,{timeprices=none}))"];
  M0F0 -> M1F0 [label="have[tp_v = ...]"];
  M0F0 -> M3F0[label="closed"];
}
\end{dot2tex}
\caption{Fusing pull instructions; the first process is pulling from a local stream, while the second process is trying to pull from a connected stream produced by the first.}
...
digraph G {
node[shape=none];
  M1 [texlbl="\colorbox{procFst}{push timeprices (tp\_f tp\_v)} (M1)"];
  M2 [label="... (M2)"];
  M1 -> M2;
}
\end{dot2tex}
...
digraph G {
node[shape=none];
  F0 [texlbl="\colorbox{procSnd}{pull timeprices reg\_v} (F0)"];
  F1 [label="... (F1)"];
  F2 [label="... (F2)"];
  F0 -> F1 [label="have[reg_v = ...]"];
  F0 -> F2[label="closed"];
...
digraph G {
node[shape=none];
  M1F0 [texlbl="\colorbox{procFst}{push timeprices (tp\_f tp\_v)[chan\_v=tp\_f tp\_v]} \FuTiReLa{M1}{F0}{none}"];
  M2F0 [texlbl="\colorbox{procSnd}{jump [reg\_v=chan\_v]} \FuTiReLa{M2}{F0}{pending}"];
  M2F1 [label="... ((M2,{}),(F1,{timeprices=have}))"];
  M1F0 -> M2F0 [label="[chan_v = tp_f tp_v]"];
  M2F0 -> M2F1 [label="[reg_v = chan_tp]"];
}
\end{dot2tex}
\caption{Fusing a push with a pull}
\label{figs/fsm/fuse-pulls}
\end{figure}
...
Both the @timeprices@ and @regression@ processes pull from the same stream as their initial instruction, so we have the situation shown in the top of Figure~\ref{fig:Fusion:Pulls}.
The @group@ machine needs to transition from label @A0@ to label @A1@, and the @merge@ machine from @B0@ to @B1@.
In the result machine we produce three new instructions that transition between four joint result states, @F0@ to @F3@.
Each of the joint result states represents a combination of two source states, one from each of the source machines.
...
We also associate each of the joint result states with a description of whether each source machine has already pulled a value from each of its input streams.
For the @F0@ case at the top of Figure~\ref{fig:Fusion:Pulls} we have ((A0, \{sIn1 = none\}), (B0, \{sIn1 = none, sIn2 = none\})).
The result state @F0@ represents a combination of the two source states @A0@ and @B0@.
As both @A0@ and @B0@ are the initial states of their respective machines, those machines have not yet pulled any values from their two input streams, so both `sIn1' and `sIn2' map to `none'.
...
Without draining, the producer of the unfinished input stream would continue pushing, potentially requiring an unbounded buffer; we look at draining in more detail in \REF{kpn/draining}.
