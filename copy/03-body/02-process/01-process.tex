...
\begin{process}[float,caption=Process implementation of foldl,label=figs/procs/impl/foldl]
foldl 
  = \ (k  : b -> a -> b) (z   : b) (j : b -> c)
...
\begin{process}[float,caption=Process implementation of map,label=figs/procs/impl/map]
map 
  = \ (f  : a -> b)
      (sIn: Stream a) (sOut: Stream b). 
    / (v  : a)        (M0..M4: Label).
...
\begin{process}[float,caption=Process implementation of join,label=figs/procs/impl/join]
join 
  = \ (cmp : a -> b -> Ordering)
...
% -------------------------------------
\smallskip
\Cref{fig:Fusion:Def:Step} defines function \ti{tryStep} which schedules a single instruction. This function takes the map of channel types, along with the current label and associated instruction of the first (left) process, and the current label of the other (right) process.
