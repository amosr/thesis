...
But is this actually useful? Is it mainly an aesthetic thing?
What do we gain from keeping the two processes synchronised like this?
One thing is that by synchronising the two processes together, when we fuse we will only have one copy of the code that pulls.
So because @P@ can only start pulling again by the time @Q@ has dropped, this means @P@ and @Q@ must both be trying to pull at the same time, which means we can reuse the same instructions generated from the previous time they both pulled.
