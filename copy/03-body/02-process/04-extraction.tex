...
In one order, we could fuse the two @append@ processes together, then fuse with the @zip@ process.
The other two orders fuse the @zip@ process with one of the @append@ processes, then fuse with the other @append@ process.
...
Fusing the two @append@ processes together caused us to prematurely commit to an interleaving that is locally correct, but globally wrong.

Fusion succeeds in the other order, where we fuse the @zip@ process with one of the @append@ processes first, then fuse with the other @append@ process.
...
This technique for determining fusion order is sound.
Regardless of the order the processes are fused in, if fusion succeeds, the result process has the same meaning.
This fusion order works for all examples we have tried \REFTODO{benchmarks}, but it is not complete.
...
Because we have not proved completeness, there may be process networks for which some fusion order would work, but for which technique would choose the wrong order which fails to fuse.

Sometimes there is no fusion order, even though there should be.
Some process networks which we can fuse by hand, cannot be fused in any order at all.
...
There is no single consumer in this example which imposes its access pattern on its producers.
No matter what order we try to fuse the processes, it will fail if the arbitrarily-chosen interleaving .


We propose to solve this in future work \REFTODO{future of fusion} by modifying the fusion algorithm to work for any order.
Rather than knowing that there exists always a order which would work, we can apply fusion in any order knowing that all orders produce the same result.

\TODO{Are there cases which should fuse, but do not, regardless of the order?}
