...
For this example we shall write a program to read input lines from a file, filter out all except those starting with the string \verb/"Good"/, and write the results to file.
...
Inside the definition, the Template Haskell splice \verb/$$(fuse ...)/ takes the process network as an argument, fuses the processes together, and generates output code.
...
Now we take the values in @input@ and filter them to those starting with the string \verb/"Good"/.
...
Show the process network and the processes.
