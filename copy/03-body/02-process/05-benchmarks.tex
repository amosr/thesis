...
Our Template Haskell implementation does not have access to the variable names used by the original program, so we denote the channels by the placeholder names \Hs@C0@ to \Hs@C5@.
...
The \Hs/partition/ process consumes the \Hs/C0/ stream and produces the output channels \Hs/C1/ and \Hs/C2/, respectively named \Hs/evens/ and \Hs/odds/ in the original program.

The fused process network shows that the \Hs/append/ process has been fused with the two \Hs/map/ processes, but could not be fused with the \Hs/partition/ process.
...
To implement this in a single traversal of the input stream, we would need to read from the input stream \Hs@x0@, and if we see an odd-valued element, we would need to hold on to it until we were sure there were no even-valued elements left.
...
Folderol & 2-source & 0.42s &   100\% & 8.0e7 & 100\% \\
         ...
         & concurrent (partially fused) & 0.85s &   202\% & 2.7e9 & 3,418\% \\
Vector   & 2-source & 0.41s &    98\% & 1.6e8 &  200\% \\
         & 2-loop   & 0.31s &    74\% & 1.6e8 &  200\% \\
\end{tabular}
% KPN execution:
%                   chunk size   runtime -N1    runtime -N2   runtime -N4       alloc
% partially fused            1             -              -             -           -
%                           10           640              -             -           -
%                          100           456              -             -           -
%                         1000           385            220             -           -
...
The partially fused Folderol version, which executes as a concurrent process network, is the slowest and allocates the most memory, because the elements must be allocated .
