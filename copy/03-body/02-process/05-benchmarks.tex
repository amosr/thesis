...
In contrast, in Pipes/Conduit, the step function and state are mixed together: the entire stream is only available dynamically as a function closure, hidden inside a constructor inside a monadic operation.
...
\begin{lstlisting}[float,label=l:bench:filterMaxType,caption=Type signature for filterMax]
filterMax :: Line -> Vector Point -> IO (Point, Vector Point)
\end{lstlisting}

\autoref{l:bench:filterMaxType} shows the type signature for @filterMax@.
...
\begin{lstlisting}[float,label=l:bench:filterMaxVectorStore,caption=Vector / store implementation of filterMax]
...
\begin{lstlisting}[float,label=l:bench:filterMaxVectorRecompute,caption=Vector / recompute implementation of filterMax]
...
\begin{lstlisting}[float,label=l:bench:filterMaxConduitTwoPass,caption=Conduit 2-pass implementation of filterMax]
...
\begin{lstlisting}[float,label=l:bench:filterMaxConduitOnePass,caption=Conduit one-pass (hand-fused) implementation of filterMax]
...
\begin{lstlisting}[float,label=l:bench:filterMaxStreaming,caption=Streaming implementation of filterMax]
...
These streaming libraries are not usually used for array computations, and it should now be obvious why.
...
\begin{lstlisting}[float,label=l:bench:compressorType,caption=Type signature for compressor]
...
\autoref{table:bench:quickhull} shows the runtimes for the dynamic range compressor over $10^8$ samples, which corresponds to around 750MB in memory.
