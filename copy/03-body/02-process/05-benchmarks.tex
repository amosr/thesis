...
priceAnalysesFolderol :: (FilePath,FilePath) -> IO ((Double,Double), (Double,Double))
...
\Cref{fig:runtime:gold-panning} shows the results of benchmarking the different versions of \Hs/priceAnalyses/ over $10^6$ elements.
...
% We discuss the kinds of programs that fusion failure in more detail in \cref{s:Benchmarks:FusionFailure}.
\FigurePdfLabel{figs/depgraphs/partitionAppendFailure}{figs/dep/partitionAppendFailure}{Dependency graph for \Hs/partitionAppendFailure/}

% Consider the dependency graph for \Hs/partitionAppendFailure/ shown in \cref{figs/dep/partitionAppendFailure}.
...
\Cref{figs/dep/partitionAppendFailure} shows the dependency graph for \Hs/partitionAppendFailure/.
...
Deadlocks can also occur when two distinct sets of processes have more than one input stream in common.
...
The Vector library's implementation of stream fusion has faced similar problems with exponential blowup, as it relies on the constructor specialisation optimisation \citep{peyton2007call} to generate efficient code.
...
% \section{Fusion failure}
% \label{s:Benchmarks:FusionFailure}
% \cref{s:Benchmarks:ResultSize}
