...
If the volume of the input signal is not above the volume threshold, the transfer function sets the gain multiplier to one, indicating no change.
...
The combinators \Hs/square/, \Hs/root/ and \Hs/transfer/ are computed by applying a single function to each stream element.
...
Here, we use \Hs@postscanl@ to create a low-pass filter to remove frequencies above 20kHz, and call the result stream \Hs@lopass@.
...
For the Vector implementation in the previous example, the two occurrences of \Hs@ins@ just meant that the input array was indexed twice.
However, in the modified version with low-pass (\cref{l:a:bench:compressorLopVector}), it is not an input vector being used twice, but the result of \Hs@postscanl@.
This means that the result vector for \Hs@lopped@ must be reified into a manifest vector before it can be reused.
