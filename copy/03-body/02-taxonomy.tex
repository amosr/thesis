...
Although we use a slightly different representation, the push streams described here are analogous to the \emph{sinks} described in \cite{bernardy2015duality} and \cite{lippmeier2016polarized}.
Push streams should not be confused with \emph{push arrays}, for example in \cite{claessen2012expressive}.
...
Like pull streams, push arrays do not support executing multiple queries concurrently; nor do they support combinators with multiple inputs except append.
We discuss push arrays further in \cref{related/push-arrays}.
