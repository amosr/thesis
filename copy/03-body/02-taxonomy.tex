...
The comparison of stock price to market index price compares each day's stock price against the corresponding day's market price.
Each day is visualised as the percentage difference between the day's stock price and the mean stock price for the period for the y axis, compared to the percentage difference between the day's market price and the mean market price for the period for the x axis.
...
The \Hs/join/ operator matches each stock day against the corresponding market index day, and discards any days missing from either input.
...
This pull stream representation can incur some streaming overhead because of the allocated \Hs@Maybe@ values.
...
To demonstrate concretely the overhead of composing stream transformers, we take the definition of \Hs/Pull.map/ and inline it into the use-sites in \Hs@bs@ and \Hs@cs@ above.
...
Stream fusion \citep{coutts2007stream}, which uses a more sophisticated pull stream representation, allows pull functions to return a \emph{skip} constructor, instructing the consumer to pull again.
...
We achieve this duplication by constructing a pull stream which, when pulled on, pulls from its source \Hs@push_a@, pushes the value to sink \Hs@push_b@, and returns the value to the caller.
...
All these systems have one place driving the computation.
...
Concurrent programs can be hard to write and debug because the \emph{schedule}, which specifies the interleaving of process execution, depends on the environment outside of the process.
\TODO{be more specific about environment}
...
Kahn process networks are a restricted form of static process network where all processes compute the same result, independent of the schedule \cite{kahn1976coroutines}: the schedule may be chosen non-deterministically, but the result is still deterministic.
...
The Kahn process network model specifically rules out processes with shared mutable state, as they can non-deterministically compute different results.
...
Only one process is allowed to push to a channel: if two processes were able to push to the same channel at the same time, the scheduler would have to decide the order in which values were received.
...
For \Hs@priceAnalysis@, our algorithm can automatically combine all the processes together into a single processes.
A single process executes sequentially; fusing the entire network into a single process removes any potential speedup from task parallelism, but in our benchmarks in \cref{s:Benchmarks}, the sequential version is faster than the concurrent version even with several processors.
