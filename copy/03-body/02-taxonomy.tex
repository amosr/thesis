...
\end{dot2tex}
\caption{Dependency graph for queries priceOverTime and priceOverMarket}
...
However, the `push' in `push streams' is different from the `push' in the `\emph{push model}' used for database execution, as described in \citet{neumann2011efficiently}.
In the Neumann push model, a stream producer is represented as a continuation which takes a sink to push values into.
...
\begin{haskell}[float,caption=Polarised implementation of priceOverTime and priceOverMarket,label=figs/impl/polar/priceOverTime-priceOverMarket]
...
\end{dot2tex}
\caption{Polarised dependency graph for priceOverTime correlation using zip}
\label{figs/polar/correlation-zip}
\end{figure}

\begin{haskell}[float,caption=Polarised implementation of zip\_ioo,label=figs/polar/impl/zip_ioo]
...
\begin{haskell}[caption=Types and combinators for Kahn process networks,label=figs/kpn/combinators]
data Channel a
...
\begin{haskell}[caption=Implementations of priceAnalyses queries as a Kahn process network,label=figs/kpn/impl/priceAnalyses]
