...
The Pearson correlation coefficient is defined as the covariance of both axes, divided by the product of the standard deviations of each axis separately.
We can also define the Pearson correlation coefficient as the angle between the two regression lines: regression line of price over time, and the regression line of time over price.
% The exact details are not relevant; it is sufficient to know that these analyses can be implemented as a single-pass online algorithm.
...
In this graph, the two linear regression lines represent .
the correlation is represented by the angle between the red and blue regression lines; the smaller the angle between the two regression lines, the more closely correlated the two are and the straighter the relationship is.
