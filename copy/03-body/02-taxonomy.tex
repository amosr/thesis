...
This is discussed further in \REF{related/fusion}.
...
When we have multiple queries to execute, it can be hard to choose which one should control the computation.

An alternate way to execute multiple queries at the same time is to use multiple processes of control, as in a \emph{Kahn process network}, a kind of concurrent process network.
Each combinator inside each query, as well as each input stream, becomes a communicating process in the network.
Processes communicate through input channels which they can pull values from, or output channels which they can push values to.
...
Concurrent programs can be hard to write and debug because the scheduler, which interleaves execution of the processes, appears to behave non-deterministically from the perspective of the processes.
Programs may give different results for different schedules.
Kahn process networks are a restricted form of process network that ensures processes execute deterministically, regardless of how processes are scheduled \cite{kahn1976coroutines}.
Deterministic evaluation is achieved by imposing restrictions on how processes communicate; scheduling decisions cannot be observed.
The restrictions are: all communication between processes is through first-in-first-out channels; reading from channels is blocking; and channels can only be written to by a single process.
The first restriction ensures processes cannot communicate via shared mutable state: if one process is reading from mutable state while another is writing a new value, the reading process can get the old value or new value, depending on how the processes are scheduled.
Processes with shared mutable state can non-deterministically compute different results.
Similarly, a process cannot \emph{peek} at a channel to see whether there are waiting values, because another process might be waiting to be scheduled and about to push a new value.
Finally, only one process can push to a channel, because if two processes tried to push at the same time, the scheduler would have to decide the order the values were received.

With Kahn process networks, we can implement a process which joins two streams sorted by date, and we can share streams among multiple consumers by broadcasting values to multiple output channels.
...
We leave discussion of implementation details for \REF{kpn}, and for now focus solely on the interface.
The @Channel@ type denotes a communication channel between processes.
The @Network@ monad describes how to construct a process network, although actual execution is deferred until after the entire network has been constructed.
The @map@ and @join@ combinators have type signatures similar to list versions, with lists replaced by @Channel@s and the return value inside the @Network@ monad.

Because execution is deferred, the @foldl@ combinator cannot return the fold result immediately; instead, it wraps it in a @Result@ type.
...
\autoref{figs/kpn/impl/priceAnalyses} shows how an implementation of the @priceAnalyses@ queries with Kahn process networks.
There are some differences from the list version because the process network is constructed inside the @Network@ monad and the results are paired together using @Result@ applicative functor instance.
