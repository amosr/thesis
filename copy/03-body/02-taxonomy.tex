...
In \autoref{related} we will compare with some more expressive but less efficient streaming models.
...
\autoref{fig:stock:it:stock-over-time} shows the fluctuations of the example stock's price over a year, along with the regression line of price over time in red, and the regression line of time over price in blue.
...
\autoref{figs/impl/correlation} contains the implementations of the fold worker functions \Hs/correlation_x/, \Hs/correlation_k/ and \Hs/correlation_z/.
...
\autoref{figs/impl/pull/combinator} shows the type signatures of the push stream versions of \Hs/regression/ and \Hs/correlation/, as well as the implementation of the \Hs/join/ combinator.
...
This is a simple instance of a transform known as \emph{tupling}.
Transforms such as \cite{hu1997tupling,hu2005program,chiba2010program} can automatically perform tupling for some programs, but do not support combinators with multiple input streams such as \Hs/join/, \Hs/zip/, or \Hs/append/.
...
We shall see more examples of push programs in \autoref{part:icicle}.
...
Concurrent programs can be hard to write and debug because the \emph{schedule}, how processes are interleaved during execution, depends on the environment.
...
We leave discussion of the implementation for \REF{kpn}, and for now focus solely on this simplified version of the interface.
...
In \autoref{taxonomy/pull/streaming-overhead} we saw that communication between pull stream combinators involves allocating a @Maybe@ value, which can sometimes be removed by general purpose compiler optimisations.
...
After looking at Icicle, we shall see how Kahn process networks can be executed efficiently by fusing processes together \REF{kpn}.
