...
This implementation is more complicated than the list version, but it does execute in a streaming single-pass.
The control flow is represented backwards in the places where push streams are used and forwards where pull streams are used.

\begin{figure}
\begin{lstlisting}
priceOverTime_polar :: IO (Push Record (Line,Double))
priceOverTime_polar = do
  reg   <- regression_o
  cor   <- correlation_o
  let cm = map_o (\r -> (daysSinceEpoch (time r), price r))
                 (Push.dup_ooo reg cor)
  return cm

priceAnalyses_polar :: PullResult Record r -> PullResult Record r'
                    -> IO ((Line,Double), (Line, Double))
priceAnalyses_polar stock index = do
  pot        <- priceOverTime_push
