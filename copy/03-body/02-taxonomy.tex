...
For this reason, pull streams, as well as Java 8 Streams \citep{reese2014java} and Strymonas \citep{biboudis2017expressive}, require that streams are not used multiple times.
...
Some models, such as Iteree-based streams \citep{kiselyov2012iteratees}, do not explicitly outlaw streams from being used multiple times, but can exhibit the same surprising behaviour when multiple consumers take elements from the same stream.
...
In order to rewind the stream, all the effects and all the work that went into computing the stream the first time must be done a second time.
...
Real-time inputs such as sensor data may be too large to store indefinitely, and for network-backed storage, the extra communication time may make performing multiple reads impractical.
...
We could also use the applicative functor instance for push streams to combine consumers together, specifying how to transform and combine the results.
...
The incomplete implementation in \cref{figs/polar/impl/priceOverTime_c} also demonstrates the recursive dependency between @stock'@, @times@, @prices@ and @cor@.
