...
As we shall see in \autoref{s:extraction:future},
...
The names of each stream and process are written above the line, and to fit on the page the names of the @append@ processes are written using (@++@), the list-append operator.
...
When input stream @b@ pushes its elements, it pushes to both its consumers at the same time.
