...
To see the effect of clustering, consider how we might execute the following array program as a stream program:
...
The function \Hs@normalize2@ computes two sums: one of all the elements of \Hs@xs@, the other of only elements greater than zero.
...
This clustering combines the \Hs@sum1@ fold and \Hs@sum2@ filter into one loop, but requires an extra loop for the fold operation which consumes the filter output, since it cannot fuse filter operations.
