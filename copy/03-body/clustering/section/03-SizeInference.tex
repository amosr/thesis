...
The \Hs/sum1/ binding does not bind any array variables and works for any input size, so we leave the environment as-is and produce a $\true$ constraint.
For the \Hs/gts/ binding, the size of the output array is unknown.
...
For the \Hs/ys1/ binding, the size of the output array is the same as the input array.
...
In the constraints, we require all three variables to be equal, ensuring that array size variables occur on the left-hand side and unification variables on the right.
Constraint generation for the remaining bindings proceeds similarly.


% -------------------------------------------------------------------
\subsection{Constraint solving and generalization}
The top-level judgment form ($\SizeF{\program}{\sigma}$) assigns a size scheme to a program by extracting size constraints and then solving them.
The top-level rule in \cref{clustering:f:ConstraintSolving} assigns a size scheme to a program by first extracting size constraints, before solving them and generalizing the result.
...
Recall from the previous section that in our generated constraints all the size variables named after program variables are on the left of atomic equality constraints, while all the unification and existential variables are on the right.
...
Note that $k_{xs}$ is mentioned twice on the left of an atomic equality constraint, so we can substitute $k_2$ for $k_3$. Eliminating the duplicates, as well as the trivially $\true$ terms then yields:
$$
\begin{array}{ll}
   & @xs @ : k_{xs},~
@gts@ : k_{gts},~ \exists k_1,~
...
At this stage we are stuck, because the constraints are not yet in solved form, and we cannot simplify them further. Both $k_1$ and $k_2$ are marked as rigid, so we cannot substitute one for the other and produce a single atomic constraint for $k_{flt2}$.
...
After inferring the size of each array variable, each operator is assigned an \emph{iteration size}, which is the number of iterations needed in the loop which evaluates that operator. For \Hs@filter@ and other size changing operators, the iteration and result sizes are in general different. For such an operator, we say that the result size is a \emph{descendant} of the iteration size. Conversely, the iteration size is a \emph{parent} of the result size. 
...
Looking back at the \Hs@normalize2@ example, the iteration sizes of the combinators of \Hs@gts@ and \Hs@sum1@ are both $k_{xs}$.
...
The iteration size of \Hs@sum2@ is $k_{gts}$, and the filter combinator which produces \Hs@gts@ is a transducer from $k_{xs}$ to $k_{gts}$. 
Even though $k_{gts}$ is not equal to $k_{xs}$, the three nodes \Hs@gts@, \Hs@sum1@ and \Hs@sum2@ can all be fused together.
...
In the previous example, \Hs@sum1@ and \Hs@sum2@ have different iteration size, and their parents are $parents(\Hs@sum1@, \Hs@sum2@) = \{(\Hs@sum1@, \Hs@gts@)\})$.
In order for \Hs@sum1@ and \Hs@sum2@ to be fused together, \Hs@sum1@ and \Hs@gts@ must be fused, \Hs@sum1@ and \Hs@sum1@ must be fused, and \Hs@gts@ and \Hs@sum2@ must be fused. Now we can express the restriction on programs we view as valid for our transformation more formally:
