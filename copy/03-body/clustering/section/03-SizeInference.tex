...
With the $\trans$ function, we can express the restriction on programs we view as valid for our transformation more formally:
...
Informally, only the $\Hs/map/_n$ clause in $\trans$ can introduce multiple transducers in the result, and only the \Hs/filter/ clause can introduce a transducer.
...
$\parents'(bs, a, b)$ \\
        \> $|$ \> $\iiter_{\Gamma,C}(bs(a)) == \iiter_{\Gamma,C}(bs(b))$ \\
        \>     \>                      \> $=$ \> $\{(a, b, 0)\}$ \\
        \> $|$ \> $a' \in \trans(bs,a),~ p_a \in \parents'(bs,a',b)$ \\
        \> $,$ \> $b' \in \trans(bs,b),~ p_b \in \parents'(bs,a,b')$ \\
...
Although we use the terms ``ancestors'' and ``parents'', if the two input nodes have the same iteration size, the two parents will be the inputs themselves and the above fusion requirements for different size inputs are trivially satisfied, since a node is always fused with itself.

The definition of $\parents'$ returns the pair of parents as well as the distance, determined by counting how many other ancestor transducers there are between the input node and the parents.
The $\mi{closest}$ function compares the distances of two pairs of parents and chooses the closest pair.
The $\mi{increment}$ function increases the distance by one.

A single combinator can have multiple ancestor transducers, though the $\trans$ function returns only the parent transducer.
For a pair of nodes, there may be many different pairs of ancestor transducers with compatible iteration sizes.
In general, the pair of nodes can be fused together if \emph{any} pair of ancestor transducers with compatible iterations sizes are fused together.
Because fusion is transitive, we can express this by requiring the pair of nodes to be fused with the closest pair of ancestors only.
