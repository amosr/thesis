...
Before performing clustering, we need to infer the relative sizes of each array in the program to determine which array operations can be assigned to the same cluster.
The array operations in a cluster are fused into a loop with a specific number of iterations, so array operations that process different sized arrays cannot usually be assigned to the same cluster.
Our $\Hs/map/_n$ combinator requires all input arrays to be the same size, and returns an output array of the same size.
...
This section presents a simple constraint-based inference algorithm.
