...
To demonstrate the ILP formulation we will use the @normalize2@ example from \cref{clustering:s:Introduction}, repeated here:
\begin{code}
...
We use the final ILP formulation from \cref{clustering:s:OptimisedConstraints}. First, we calculate $possible$ -- that is, the nodes which have no fusion-preventing path between them.
\[ \{ \{sum1, gts, sum2\}
 , \{sum1, ys2\}
 , \{gts, sum2, ys1\}
 , \{ys1, ys2\} \} \]
