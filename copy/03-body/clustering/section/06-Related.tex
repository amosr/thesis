...
Recent literature on array fusion for imperative languages largely focuses on the polyhedral model.
This is an algebraic representation imperative loop nests and transformations on them, including fusion transformations.
Polyhedral systems \cite{pouchet2011polyhedral} are able to express \emph{all possible} distinct loop transformations where the array indices, conditionals and loop bounds are affine functions of the surrounding loop indices.
...
% -----------------------------------------------------------------------------
% \subsection{Haskell shortcut fusion}
...
% 
% Consider the following \Hs@filterMax@ function, where each element in the input array is % incremented, then the maximum is found, and the array is filtered to those greater tha% n zero.
% In this case, the result of the map \Hs@vs'@ cannot be inlined into both occurences witho% ut % duplicating work, so no fusion can be performed. This has the effect of performing thr% ee % loops % instead of one, with two arrays instead of one.% 
% 
% \begin{code}
% filterMax vs =
%  let vs' = map    (+1)  vs
