...
\ifprint
  \colorlet{CommentStyle}{black!80}
  \colorlet{StringStyle}{black!80}
  \colorlet{KeywordStyle}{black!80}
  \newcommand\KeywordStyle \textbf

\else
  \colorlet{CommentStyle}{MidnightBlue}
  \colorlet{StringStyle}{Maroon}
  \colorlet{KeywordStyle}{OliveGreen}
  \newcommand\KeywordStyle \tt
\fi

\colorlet{Highlighted}{green!10}
...
\lstset{%
    ...
    commentstyle=\color{CommentStyle}\it,
    stringstyle=\color{StringStyle},
    keywordstyle=[1]\color{KeywordStyle}\KeywordStyle,
...
\lstdefinelanguage{lucy}%
  {morekeywords={let,and,if,then,pre,or,node,done,unless,run,end,rec,where,fun,else,not,open,match,automaton,continue,emit,when,fby,merge,reset,every,do,ntil,on,await,type,in,whenot},%
   keepspaces,%
   sensitive,%
   morestring=[b]",%
  % moredelim=**[is][\color{TemplateStage}]{@}{@},
...
\lstnewenvironment{lucy}[1][]{\lstset{language=lucy,#1}}{}
\lstnewenvironment{listing-c}[1][]{\lstset{language=C++,morekeywords={size_t},literate=,#1}}{}
...
\def\Lucy{\lstinline[language=lucy]}
